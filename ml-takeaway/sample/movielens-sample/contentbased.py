...
class ContentBasedRecommender(object):
    def __init__(self, dataset_handler):
        ...
        self.books_vectors = self.dataset_handler.load_movies()
    ...
    def create_user_profile(self, user_ratings):
        ...
        return (
            np.average(
                np.array([
                    np.array([1] * len(user_ratings.items()))
    ...
    def present_user_profile(self, user_profile):
        ...
        for (movieId, rating) in user_profile[1].items():
            movie_vector = self.books_vectors[self.dataset_handler.id2index(movieId)]
            print("{} {}: {}".format(
    ...
    def _cosineKNN_movies_subset(self, movies_subset, movieId, k):
        ...
        movies_with_ids = np.array([
            np.hstack([[watched_movie], self.books_vectors[self.dataset_handler.id2index(watched_movie)]])
            for watched_movie in movies_subset
        ])
        nbrs.fit(movies_with_ids[:, 1:])
        return movies_with_ids[
            nbrs.kneighbors(
                np.array([self.books_vectors[self.dataset_handler.id2index(movieId)]]), return_distance=False
            )[0],
            0
        ]
...
user_profile = recommender.create_user_profile(user_ratings[26713])
